...
rm -rf $CSV_OUTPUT
> $CSV_OUTPUT
echo "TIME_STAMP, CPU_1 (%), CPU_2 (%), MEM (kB)" | tee -a $CSV_OUTPUT
...
do
	...
	CPU_1=$(top -b -n 1| grep -w mongod | head -1 | tr -s ' ' | cut -d ' ' -f 9)
	CPU_2=$(top -b -n 1| grep -w mongod | tail -1 | tr -s ' ' | cut -d ' ' -f 9)
	#CPU=$((CPU_1 + CPU_2))
	#if [[ $CPU != *"."* ]]; then
       	# 	CPU=$(top -b -n 1| grep -w mongod | tr -s ' ' | cut -d ' ' -f 10)
    	#fi
	MEM=$(sudo pmap -x $(pgrep mongo) | tail -1 | grep -o -E '[0-9]+' | head -1)
	echo "$DATE, $CPU_1, $CPU_2, $MEM" | tee -a $CSV_OUTPUT	
done
